    ...
    def on_release(self, event):
        if event.inaxes:
            ...
            if self.sub_df is not None:
                self.sub_df = self.sub_df.append(
                    self.df[(self.df['timestamp'] > self.min_x) & (self.df['timestamp'] < self.max_x) & \
            ...
            print('Selected (%d < timestamp < %d) and (%d < amplitude < %d)' % (
...
class App(object):
    def __init__(self, window, ip, user, pswd, db):
        ...
        Button(window, textvariable=self.query_button, command=self.sendQuery).grid(row=self.current_row, column=0,
                                                                                    columnspan=2)
        self.current_row += 1
    ...
    def sendQuery(self):
        ...
        if (start_t) and (not time_format.match(start_t)):
            messagebox.showinfo('Query Error',
                                'Error: Start time Must have the format "YYYY-MM-DD HH:MM:SS" or be empty')
